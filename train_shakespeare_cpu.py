...
# Hyperparameters for "Small Router, Large Pool" on CPU
batch_size = 32
block_size = 64
max_iters = 500  # Short training for demonstration
eval_interval = 100
learning_rate = 1e-3
...
# DPSN Specifics
pool_size = 100_000  # Large pool
router_hidden_dim = 32  # Small router
min_params = 100
max_params = 1000
...
def train_shakespeare_cpu():
    ...
    config = LanguageModelConfig(
        ...
        n_embd=n_embd,
        n_head=n_head,
        n_layer=4,  # Small number of layers for CPU
        block_size=block_size,
        pool_size=pool_size,
        dropout=dropout,
    )

    # Override DPSN config params that are passed via kwargs in model init
    # LanguageModelConfig now has these fields (added in config/hyperparameters.py)
    ...
    # Optimizer Setup
    # 1. Router & Attention & Embeddings (AdamW)
    # 2. Pool (SGD for strict sparsity updates)
    ...
    # Save model
    torch.save(model.state_dict(), "dpsn_shakespeare_cpu.pth")
    print("Model saved to dpsn_shakespeare_cpu.pth")


if __name__ == "__main__":
    train_shakespeare_cpu()
